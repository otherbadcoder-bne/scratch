...
*   **Domain Verification:** To enable secure web access, a simple DNS record must be created in your domain's settings to validate the SSL certificate.
*   **Custom Domain Linkage:** After the wiki is deployed, a CNAME record will need to be configured with your DNS provider to connect your chosen domain name to the CloudFront distribution.
