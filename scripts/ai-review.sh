...
# Guard first — skip expensive context discovery if there's nothing to review
...
TIMEOUT=120
RESULT=$(perl -e "alarm(${TIMEOUT}); exec(@ARGV)" -- gemini -m gemini-2.5-flash --approval-mode plan -p "$(cat "${PROMPT_FILE}")" 2>/dev/null) || {
  CODE=$?
  if [[ $CODE -eq 142 ]]; then
