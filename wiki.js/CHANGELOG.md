...
### Security decisions
- **No WAF** — AWS WAF adds ~$6-10/month minimum (WebACL + rule groups) which nearly doubles the baseline cost of this deployment (~$12/month). For a small internal wiki behind Google OAuth, the cost-benefit does not justify it. This will be reconsidered if the wiki becomes more widely used or handles sensitive data.
